...
- **Teacher names: "Ms. Jayanthi/Mr. Matt"**, always both. A student has one teacher or the other, so a bare "Mr. Matt" reads as though Jayanthi's sections do not exist. The `welcome.njk` signature reads "Mr. Matt and Ms. Jayanthi"; the ordering difference is deliberate. **History: renamed to "Mr. Matt" on 2026-08-07, reversed to "Mr. Ignash" on 2026-08-19 (0ac18e2), and settled on "Mr. Matt" on 2026-09-13 (121 occurrences). Do not reintroduce "Ignash"; the build gate fails on it.** Watch for first-person voice too ("while I look at it"), which a name grep will not catch.
...
- **Concepts sections** are headed "Before you start the work" and their instruction line must open with one of four stems: `Pick one` / `Start with the X` / `Use all of these` / `Keep these open`. **`npm run build` runs `scripts/check.mjs` first and fails the build if a line contradicts its cards.** That gate is the main safety net in this repo. Since 2026-09-13 it also enforces six house rules: no "Ignash"; no `href="#"` on a submit link; no raw hex outside `:root`; no font-size off the 7-step scale (print CSS exempt); no bare `<details>` (use `partials/disclosure.njk`); no "Section A/B/C", trial, arm or pilot on student pages.
...
1. **Before 10/11 August, outside this repo:**
   ...
   - **Drive documents must match the site's "Mr. Matt" (2026-09-13).** Check at least the **Syllabus 2026-27**, **Welcome to the Year**, the **U0 Unit Roadmap (for Google Classroom)**, and the **U0 Source Dossier Annotated Exemplar Set**; after the 08-19 reversal they may say Ignash. The syllabus is the one families read.
...
2. **`classroomLinks.json` is entirely empty**, 58 keys including `reflection-journal`. Every submit button and the reflection callout render their "link posted in August" fallback until filled. **The Systems Diagrammer is the one live partner** (8 September): `classroomActive: true` in its frontmatter swaps the "coming soon" placeholder for "open it from Google Classroom". Its Flint and BoodleBox links live only on the Block 2 Classroom post and are not in this repo.
...
4. ~~**Em dashes outside units**~~ **Done (Addendum 42).** All 852 removed across 54 files; Foundations, AI Partners, Reference, Student Hub and the unit pages are clean, and so are the Unit 0 student-facing documents. **What is left is deliberate:** 228 en dashes that are numeric ranges, where an en dash is correct and `pp. 8-10` would be wrong as "8 to 10", and the three simulator pages under `assets/simulators/`, which are **synced from sibling repos** by `npm run sync-simulator` and would be overwritten on the next sync. Fix those in their source projects, not here.
