...
2. **The two-texts page must never say both texts were AI-written.** `/units/unit-0/readings/two-texts/`. It says only that one carries an AI-use statement and one does not. The reveal happens in class. No spoiler in copy, no teacher note, **no HTML comment**. The two cards share one CSS class so no styling difference becomes a clue; **do not add a Text A or Text B variant.**
...
1. **Before 10/11 August, outside this repo:**
   ...
   - **The reflection seed is stale.** `Reflection Tool seed — AI disclosure prompt.xlsx`, prompt `ees-u0-b3-2`, reads "Both texts you read **today** were written with AI." The activity runs at **Block 1**, so by Block 3 students read them two blocks earlier and already have the reveal. Either reword away from "today" and from revealing something already revealed, or move it to Block 1 as `ees-u0-b1-2`. The seed file warns that Block 3's existing prompt is a callback anchor for `ees-u0-b7` and must not be retired.
