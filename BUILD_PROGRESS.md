...
**RESOLVED same day: the no-cap decision applies course-wide.** Every strand cap is gone from U1, U2, U4, U5 and U6 as well. What used to be stated as a ceiling is now stated as what the strand reads for, which is what the band descriptors already say: voice-match, stakeholder specificity, trade-off naming, quantification, generic recommendations, undifferentiated projections, the exoplanet analysis, policy/audience voice and the negotiation move all now say the work "sits in the lower/middle bands" rather than capping anything. The "gates" framing is retired with it: the U5 and U6 callouts are now "things the strands are looking for", and U4's and U2's are "what each strand reads for". **`grep` for cap phrasing now returns zero across `src/`.** AI Documentation keeps its own distinct rule: not scored until it arrives, still no cap.
...
**Addendum 24 (2026-07-30): Lovelock reading companion (U0 B2) — apparatus built; full-text hosting is an OPEN DECISION.**
- **Data**: `src/_data/lovelockGlossary.json` (copied from `../Unit 0_ Foundations/lovelock-eal-glossary.json`). 30 terms, 18 on the close-reading path, 8 with notes, 2 with translator flags (`phenomenological`, `sterile`), six languages (ko/ja/zh/fr/es/de). **All translations AI-generated and unverified**; the file's own `translationStatus` says so.
- **Page**: `src/units/unit-0/readings/lovelock-1965.njk` → `/units/unit-0/readings/lovelock-1965/`, plus `src/assets/reading-page.css`. Built and verified: the citation + "PDF is in Classroom" callout, the verbatim unverified-translations banner, a language selector persisted in `localStorage` (default `none`), the verbatim path lede, both toggles (Close-reading path default OFF, Plain-English gloss default ON), the "Read this for Origin and Purpose" group (five items incl. the OCR-error note framed as an Origin clue), the ten path paragraphs as short quoted locators each with its verbatim gloss, a 30-term tap-for-definition word list, 30 definition cards (plain + note + selected-language translation + unverified/verified marker + flag tint + course-glossary link), and a print appendix of the 18 path terms.
- **Verification support**: cards read optional per-language `verifiedBy` / `verifiedDate` from the JSON. When present for the selected language the "unverified" chip is replaced by "verified by {name}, {date}" and the warning tint drops to the SHOW-family green. **No editing UI** — verification is a JSON edit, by design. Currently 0 of 180 translation rows are verified, so every card shows the chip.
- **Print**: dimming is forced back to full opacity, controls/banner/cards are hidden, glosses print expanded, path items avoid breaking, and the appendix prints on its own page with translations omitted.
- **OPEN DECISION — the paper's full text is NOT on the page.** The prompt asked for the complete text of Lovelock (1965), *Nature* 207(4997), 568–570. That work is still in copyright (Springer Nature), and this site is **public** (public GitHub repo, public Vercel URL). Reproducing an entire copyrighted article at a world-readable URL is republication rather than classroom use, and it contradicts this repo's own standing determination in `MEDIA_REVIEW.md` ("Materials (Google Classroom, not the public site) … Public site carries framing + citations + a 'materials in Classroom' pointer only"). The page is therefore built as a **companion**: it keys every path paragraph to the PDF by its opening words, so it is usable open beside the Classroom PDF. **Consequence to know**: the term-marking filter (`markGlossaryTerms`, in `.eleventy.js`) works and is wired, but the glosses are deliberately plain English and contain none of the 30 technical terms, so in-text marks currently appear only in the word list. If the text is ever legitimately added to the page, the filter marks it with no further work. Routes forward, in order of ease: (1) leave as built; (2) clear public posting with the AISC librarian or a Springer Nature permissions request, then add the text; (3) quote the ten path paragraphs as excerpts-with-commentary rather than the whole article.
- **DECIDED (2026-07-30)**: route (1). The article and its adaptive version are Classroom artifacts; the site references them. Alongside the site page there is now a **standalone reading companion artifact** (self-contained single-file HTML, published to a private claude.ai artifact URL) that Matthew links from Classroom. It carries the same content as the site page (the verbatim banner and lede, the ten locator-plus-gloss steps, all 30 terms with six-language cards, unverified/verified chips, translator flags, both toggles, print appendix) with no site nav and no external requests, and it stores the language choice under its own `localStorage` key (`lovelock-companion-lang`). It contains no text from the paper. **If the glossary JSON changes, the artifact does not update itself** — it embeds a snapshot of the data, so republish it after editing `src/_data/lovelockGlossary.json` (source template kept alongside the build script in the session scratchpad; regenerate by injecting the JSON at the `/*GLOSSARY_DATA*/` marker).
- **Split view (final shape, 2026-07-30)**: the reading page is now two panes. Left is a Google Drive PDF viewer pointed at the school-shared copy of the article; right is the support rail (ten locator-plus-gloss steps, the Origin/Purpose five with the OCR-error note, the 30-word tap-for-definition list, language selector, both toggles). **One line of setup**: paste the Drive file ID into `driveFileId` in the page's frontmatter. Until then the page renders an explicit "Not connected yet" panel instead of a broken frame. The site only frames the file, so **Drive still enforces access** and the sharing setting stays school-only; a non-school visitor to the public page sees Drive's own access wall. Print hides the frame (a framed PDF prints from Drive, not through us) and keeps the rail plus the word appendix. **Known limits, not bugs**: because the paper sits inside a PDF frame we cannot mark glossary terms inside its text or scroll-sync the gloss to the reader's position, so the rail is keyed by each paragraph's opening words instead; and Drive iframes require a signed-in school account, with third-party-cookie and framing behaviour varying by browser, which is why the fallback "open it in a new tab" link is always present. This was **not** testable against the real file (no file ID yet), so confirm the frame loads for a student account before class.
- **OCR-error note reworded (2026-07-30, supplied final)**: the rail paragraph no longer names the two misread words. It now says the scanned PDF contains a couple of obvious scanner misreads, that the class reading tool corrects them back to the 1965 printed text, and that the fact of reading a machine's copy still belongs in the student's Origin. **Neither misread word appears anywhere in `src/`, the built site, or this file** (grep-verified); keep it that way when editing this section. Note the copy asserts that the reading tool corrects them, so that has to stay true of whichever tool students actually open.
- **Interactive reader embedded (2026-07-30)**: the left pane now frames Matthew's Apps Script web app (restricted to the school domain) via `readerUrl` in the page frontmatter, replacing the Drive PDF viewer. **Header check on the deployment**: a signed-out request 302s to Google sign-in, and that sign-in page returns `X-Frame-Options: DENY`, so Google will not render its login inside any iframe. A signed-out reader therefore sees an empty frame with no prompt of its own. The `/exec` response also carries `frame-ancestors 'self'`, so framing needs the script to opt in with `.setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL)`. The embed was tried and **failed in the browser exactly as the headers predicted** ("script.google.com refused to connect"), so the iframe is gone: the page now **links** to the reader with a "Start here" launch card that opens it in a new window, and the split layout is retired in favour of a single support column. Do not reintroduce an iframe for this URL. The deployment URL is now on a public page; it is not a credential, since Google enforces the domain restriction, but it does make the deployment ID public.
- **Word bank on the site** (added same day): `src/reference/glossary/u0-lovelock-1965.njk` → `/reference/glossary/u0-lovelock-1965/`, generated from the same JSON by the reusable `partials/word-bank.njk`. All 30 terms as anchored entries (`#term-<slug>`), each tagged with its provenance (**Unit 0 · Block 2**, plus a "close-reading path" tag on the 18) and carrying the plain definition, note, translator flag, all six translations in a table with per-language verification status, and a link back to the reading companion. Path entries take the concepts spine; off-path entries a neutral one. The glossary index gained a card for it, and the reading companion's 30 "Also in: course glossary" links now deep-link to the matching anchor (all 30 verified to resolve). Deliberately **not** 30 separate one-page entries: the existing glossary is one deep page per concept (liquefaction), and 30 thin vocabulary pages would dilute that. **To add a later reading's word bank**: add its glossary JSON to `_data` and a stub page setting `bank`, `bankUnit`, `bankBlock`, `bankReadingUrl`; the include needs no changes.
...
*A — Learning layer, with the content-match mapping (docx = U4 Block Plans, 10 sets; site = 12 blocks).* **Matched (docx set → site block, SC + bell ringer verbatim):** d1 Phenomenon launch → **B1** (site B1's LI is d1's composite) · d3 IPCC/SSP → **B5** · d5 Challenge cards → **B7** (⚠ partial: d5's SC cover the choose/lock move but not B7's LI clause about 2 sources + OPVL — Matthew may want a 4th SC) · d6 Mitigation-vs-adaptation → **B9** (its billboard bell ringer is already narrated in B9's Story prose — same activity, frontmatter is the official opener, body left per the U1 reconciliation rule) · d7 Brief drafting → **B10** · d8 Panel prep → **B11** · d9 Policy Panel → **B12**. **Orphaned docx sets (not forced, for Matthew):** d2 Greenhouse mechanism + GHGs (the site splits this content differently — B1 carries the launch LI under a greenhouse title, B2 is the energy-balance math; d2's SC fit neither LI) · d4 Climate Data Reference exploration (no site block is that day — AND the material "U4 Material — Brief Template + Climate Data Reference.docx" is referenced by zero cards; the two facts go together — candidate future block/card) · d10 Unit reflection (the site's 12-block arc ends at the panel; B12's LI absorbs a reflect clause). **Drafted (SC decomposed from LI + house-style bell ringers — FLAGGED, not from any docx): B2, B3, B4, B6, B8** — note this deviates from the prompt's "expect ~2 unmatched in B8–B12": the drafting/panel run matched cleanly; it's the science blocks the docx never covered (its arc is 10 blocks vs. the site's 12).

*B — Standards (v1, Matthew reviews).* Applied as supplied: B1/B2 [HS-ESS2-4] · B3 [HS-ESS2-6] · B4 [HS-ESS2-4, HS-ESS2-6] · B5/B6/B7 [HS-ESS3-5] · B8 [HS-ESS2-4] · B9 [HS-ESS3-6, HS-ETS1-3] · B10 [HS-ESS3-6, SEP-6] · B11/B12 [HS-ESS3-6, SEP-7]. AILit: B4 [Engage 5] · B5/B6 [Engage 3] · B10/B11 [Create 3, Manage 1]. **UbD cross-check:** the UbD's NGSS set = {HS-ESS2-2, HS-ESS2-4, HS-ESS2-6, HS-ESS3-5, HS-ESS3-6, HS-ETS1-3, SEP-2/4/5/6/7/8} — a superset of the assessment page's claimed set; the two extras are **HS-ESS2-2** (feedback loops — B8 would be its natural home if Matthew wants it tagged) and **SEP-5**; no contradictions with the applied mapping. Catalog extended with HS-ESS2-4 / HS-ESS2-6 / HS-ESS3-5 / HS-ESS3-6 (**PE texts from memory — verify against nextgenscience.org**); map-page U3 + U4 table rows now link to sections (all their codes are catalog-backed after the U3/U4 passes).

*C — Partner introductions.* Climate Modeler debuts in B1 (Story sentence + "Meet your new partner" card); Policy Aide in B5 (same treatment). **B12 did NOT reference the Defense Practice Partner in its body** (frontmatter only) — one Story line added offering last-rehearsal support [ADDITION, house-drafted].

*D — EAL.* Vocabulary-support cards (→ `/foundations/eal/u4/`) on B1, B2, B3, B6.

*E — Scaffold.* All 12 raw submit anchors → classroomLinks partial; keys `u4-b1`…`u4-b12` + `u4-brief` added (`u4-b4-footprint` kept; the parallel U3 session's `u3-*` keys preserved). **10 live-activity conversions:** B1 Interact (paired energy-balance diagrams) · B2 Watch (Stefan-Boltzmann board derivation) · B3 Interact (carbon-cycle diagram build) · B4 Interact (perturb your B3 model) · B5 Interact (whole-class OPVL on an AR6 SPM figure) · B6 Watch (live IPCC-figure walkthrough) · B7 Interact (Chennai projection figures, whole-class OPVL) · B8 Interact (ice-albedo board work) · B9 Interact (billboard sort) · B10 Interact (peer-feedback protocol). Materials named: B5 choice A now names the **Adaptation Challenge Cards** deck (printed + in Classroom); B10's Story marks the **Heat Mortality worked example** printed + in Classroom. **Revision Trail third rep added to B11's Show** ("Third run — U5 measures this; by now it should feel routine") [ADDITION — flag, mirrors U1/U2 B8].

*F — Media guard + the U4 build queue (18 placeholders remain of the original 28; none filled with unverified media — zero Dhaka/Heatscapes references placed).* Remaining, verbatim slots: **B1** Watch greenhouse-mechanism animation · **B1** Read greenhouse vocab handout · **B2** Read solar-constant/albedo lookup tables · **B3** Watch fast-vs-slow cycles animation · **B3** Read reservoir + flux quantification handout · **B4** Watch Keeling curve story · **B4** Read IPCC AR6 attribution excerpt · **B5** Watch IPCC structure + SSP primer · **B6** Read climate-model glossary · **B6** Interact curated unfamiliar-figure set · **B7** Watch why-regional-differs ← **"Three Degrees Warmer" Dhaka segment fills this when Matthew's verification watch clears (MEDIA_REVIEW: B7 primary; B4/B12 secondary)** · **B7** Read GCC Climate Action Plan excerpt · **B8** Watch tipping-points primer · **B9** Watch 3-city adaptation cases ← **"Urban Heatscapes E1" fills this when confirmed (MEDIA_REVIEW: B9 primary)** · **B9** Read adaptation-vs-mitigation glossary + GCC priorities · **B10** Watch Heat-Mortality live demo (left placeholder per the B7/B9/B10-untouched instruction; note: it's actually a live-demo candidate for `.live-activity` — MEDIA_REVIEW targets B7/B9, not B10 — convert on Matthew's nod) · **B11** Watch prior-cohort pitch (first-year ghost) · **B12** Watch prior-cohort replay (first-year ghost).

**Addendum 14 (2026-07-17): U3 structural pass — learning layer, standards, partners, EAL, scaffold (THE MOST AUTHORED PASS YET — four flag-lists below are the review queue).** All 12 U3 blocks now carry the full learning layer (LI/SC/bell ringer/standards/partners in frontmatter); both assessment pages carry Standards paragraphs; 4 raw submit anchors converted + 15 classroomLinks keys added (u3-b1…u3-b12, u3-b6-plan, u3-hazard-profile, u3-portfolio); catalog grew 15→23 entries. Build clean (149 files); all 12 blocks verified rendering bell ringer + panel + chips; "Manage 3" (uncataloged) confirmed rendering the graceful map-link fallback on B6.

- **⚠ FLAG-LIST 1 — drafted LIs (8) + ALL SC (36).** B1/B2/B3/B5 LIs migrated verbatim from their on-page choice-intros (bodies now echo `{{ learningIntention }}`). **Claude-drafted LIs: B4, B6, B7, B8, B9, B10, B11, B12** (B4 also lacked one — 8 drafted, not the expected 9; the UbD pacing table has activities but NO per-block LI statements). B4/B6/B9/B10 are completion-style per the U2 B6/B8 precedent. **All 36 SC (3 per block, including the four migrated-LI blocks) are Claude-drafted** — decomposition-only, completion-style fragments where the LI is.
- **⚠ FLAG-LIST 2 — bell ringers ×12, ALL Claude-drafted** (no U3 Block Plans docx exists). Tags: B1 phenomenon hook (epicenter-map lines) · B2 prediction (Tōhoku vs. San Andreas tsunami) · B3 retrieval (four boundary types) · B4 prediction (first-solo-sim score band) · B5 retrieval (Tōhoku three lines) · B6 prediction (hardest of three scenarios) · B7 data interpretation (82-vs-41 same-budget gap) · B8 retrieval (Sumatra-Andaman engineering failure) · B9 prediction (final score + single change) · B10 retrieval (five defense slides) · B11/B12 defense-day settlers. Priority read for Matthew's edit pass.
- **⚠ FLAG-LIST 3 — standards mapping v1 + catalog + MISMATCH.** UbD wins: primary HS-ESS2-1, HS-ESS3-1, HS-ETS1-3, **HS-ETS1-4** (not in the task's expected list); secondary HS-ESS1-5, HS-ETS1-1, HS-ETS1-2, **HS-ESS3-3** (also unexpected); SEPs 2/5/6/7. **HS-ESS2-3 (expected by the task) is NOT claimed by the UbD → not tagged anywhere, its supplied catalog entry NOT added.** HS-ETS1-1 IS claimed → added. 8 catalog additions (HS-ESS1-5, HS-ETS1-1/2/3/4, HS-ESS3-3, SEP-2, SEP-5) — **PE texts from memory/as-supplied; verify against nextgenscience.org.** Per-block v1 mapping: B1 ESS2-1+ESS1-5 · B2 ESS2-1+ESS3-1 · B3 ESS3-1+ETS1-1 · B4 ETS1-4+SEP-5 · B5 ESS3-1+SEP-6 · B6 ETS1-2+ETS1-4+AILit "Manage 3" · B7 ETS1-2+ETS1-4 · B8 SEP-6+ESS3-1 · B9 ETS1-3+ETS1-4 · B10 ETS1-3+SEP-7 · B11 SEP-7+ETS1-3 · B12 SEP-7. Assessment paragraphs: Hazard Profile = ESS2-1+ESS3-1 primary / ESS1-5 secondary / SEP-2+6; Portfolio = ETS1-3+ETS1-4 primary / ETS1-1+ETS1-2+ESS3-3 secondary / SEP-2+5+6+7. "Manage 3" is not yet in the catalog — add an entry or it stays a plain map-link chip.
- **⚠ FLAG-LIST 4 — decisions + context + build queue.** (a) **B7/B8 "introduced" context:** B7 introduces the Comparative Reflection *template*, B8 the Final Defense *format* — templates, not partners; no action taken. (b) **Live-activity conversions ×3:** B1 Do choice A (printed world-map paired plotting), B3 Do choice A (physical tech-card sort), B5 Concepts Interact (jigsaw protocol walkthrough). (c) **U3 build queue — 7 placeholders remain:** B1 Watch (interior video) · B1 Interact (cross-section interactive) · B1 choice B (digital plotting file) · B3 Watch (volcanism video) · B3 Interact (tech-card interactive) · B3 choice B (drag-and-drop) · B5 Watch (case-study-reading video). (d) **Yokohama worked-example docx exists in `../Unit 3_ Assessment Support/` but is referenced NOWHERE on the site** — Matthew decides placement. B6/B7 scaffold references now name the U3 Student Scaffolds packet. (e) **[DECISION — override by moving it] Manage 3 rehearsal lives in B6:** Story sentence + "Division-of-Labour Plan (with Iteration #1)" submission callout (id `u3-b6-plan`) added to B6's new Show section; Matthew builds the plan template next so it exists in Classroom before anyone clicks. (f) Partners: Engineering Coach added to B3/B6/B7/B9/B10 + intro (Story sentence + Meet-your-new-partner card) in B3; Plate Tectonics Tutor extended to B1/B4/B5/B8 + intro in B1; B2's "Source Evaluator" label normalized to "The Source Evaluator"; both partner pages checked — already on the dual-launch, no-system-prompt pattern, nothing to extract. (g) EAL "Vocabulary support" cards → `/foundations/eal/u3/` on B1/B2/B3/B5.
...
*⚠ THE FINDING — SC transcription mostly blocked: the Block Plans docx is a different U2 arc.* `../Unit 2_ Surface Processes/U2 Block Plans — LI + SC + Bell Ringers + Reflections.docx` (May 23) describes a 9-block arc (phenomenon launch → W/E/D trio → stratigraphy → watershed/monsoon → location cards → biography drafting → Field Geologist work → share-out prep → share-out) that does NOT match the site's built arc (weathering → erosion → rivers → coasts → Chennai+assigned → photo/map work day → worked example+OPVL → drafting+peer feedback → submitted+share-out). Same stale-docx situation as U0's block plans (see the 5→7 sync entry below). The site borrowed the docx's bell-ringer/reflection *styles* per block number (B1's reflect is docx-B1's verbatim), but the SC content diverges block-by-block. Per the "do not invent criteria" instruction, SC were added ONLY where faithful: **B2** (transcribed from its own Reflect's inline enumeration — the four-agents triplet; this also closes the Reflect's "rate yourself on each SC" reference) and **B9** (docx B9 matches the site B9 LI clause-for-clause; "biography"→"Landscape Reading" is the only edit). **B1, B3–B8 have NO SC lists — pending Matthew's call:** either update the docx to the site arc, or authorize U1-style derivation from LI clause structure + Reflect prompts (the U1 precedent in this file). B4/B8's "self-assessment vs. SC"-style reflects don't enumerate SC, so they couldn't be lifted the way B2's was.

*The two audit decisions applied (override on review if wrong):* (1) assessment AI-Doc section's "K/U cap at 3-4" → **"scores 1-2 Limited (the work isn't there to evaluate)"** (now consistent with the warn callout + B8; U0/U1 precedent); (2) propose-your-own-location deadline standardized to **"by Block 5"** (B5 Path D's "before Block 6" fixed; assessment already said Block 5; B5's Reflect "before Block 6's location lock" left alone — the lock is a B6 event, consistent).

*Everything else shipped:* Field Geologist finally introduced (B1 Story paragraph + "Meet your new partner" entry-card); Defense Practice Partner added to the unit index's partner cards; EAL Vocabulary-support cards on B1–B4 (→ `/foundations/eal/u2/`); live-activity conversions B6 Interact (peer-check projection) + B7 Watch (30-min live demo, "Recorded for replay" kept); **all nine U2 submit anchors converted from raw `href="#"` to the classroomLinks partial** — keys `u2-b1`…`u2-b9` + `u2-landscape` added to `classroomLinks.json` (empty, fill in August); B6 Path A names the Biography Template + Annotation Guide (printed + in Classroom); B7 Story notes the Pallikaranai doc is printed + in Classroom with a site version coming; **Revision Trail wired** — first run announced in U1 B8's Show ("feedback, not a grade"), second run in U2 B8's Show ("by Unit 5 this is habit"); template is Classroom-only by design.

**Addendum 7 (2026-07-15): U4 B4 gets the AI-footprint physics + the U0 AP-article callback lands.** The Engage-5 seed planted in U0 B1 (see the AP entry in `MEDIA_REVIEW.md`) pays off in U4 B4 (Human Acceleration + Warming Signal), all wording supplied-final: (1) Story paragraph connecting the week-one AP/UNU OPVL to one-way-flux physics ("448 TWh… low-carbon electricity is not automatically low-water or low-land"); (2) fourth Concepts card "Read — the numbers from week one" linking `/units/unit-0/unu-report-key-points/` (grid now exactly 4 cards — at the limit, not over); (3) second Show submission-callout **"The Footprint Decision" (assigned B4, due B5)** — one-page estimation task, printed + Classroom, wired through the submit-link partial with new `classroomLinks.json` key `u4-b4-footprint` (empty → renders the August fallback; fill in August with the rest); (4) Reflect gained the will-this-change-anything line ("'No' with a reason is a legitimate answer"); (5) U4 B9 Story: data centers added to the Chennai adaptation picture (cooling draws on the shared stressed water system) at the end of the GCC-levers paragraph — the natural spot, since B9's Story had no other demand-side passage. Note: the B4 exit-ticket callout still uses the old hardcoded `href="#"` submit link — only the new Footprint callout uses the partial; converting U4's other submit links to the partial remains an open cleanup.

**Addendum 6 (2026-07-15): U0 seven-task pass — live-activity tag (NEW SITE COMPONENT) + B1 journal setup + five smaller fills.** (1) **`.live-activity`** added to `block-page.css`: solid 2px accent border, accent-light background, white-on-accent "Live in class" tag — for cards describing live class activities that were mislabeled "Placeholder." Converted five cards (B2 Interact consensus+vocab-check; B3 Watch live-OPVL-demo + Interact side-by-side; B4 Watch; B5 Watch), dropping completed-description "TBD:" prefixes. **Use `.live-activity` for future in-class-activity cards; `.placeholder` now strictly means "asset missing."** Still placeholder on purpose: B1 orientation video + poll + social-post choice, B2's Watch model-OPVL card (not on the conversion list — flagged as a candidate), B6/B7 recording-dependent cards. (2) EAL "Vocabulary support" cards (→ `/foundations/eal/u0/`) added to B1 + B3, matching the U1 pattern. (3) B2 Read card's Classroom line renamed the scaffold plainly ("a guided walk through the paper — use it" + "If the paper feels hard, that's expected"). (4) B3 Story gained the transcript-transparency paragraph ("AI use is visible by design. Work as if your process will be read, because it will be."). (5) B1 Do section gained the "Set up your Reflection Journal (10 min, today)" callout after the choice board/energy chart. (6) B5 Reflect gained the Manage-1 seed line ("name one step of your dossier where you chose NOT to use AI"). (7) B4 + B5 Interact warm-ups made real as plain cards (B4: AP-excerpt-vs-UNU-page pair OPVL sprint; B5: mark-each-field S/V on a printed sample AI Doc entry — handout is `U0-B5 Sample AI Doc Entry — Mark It Up.docx` in the course folder, teacher key inside, content deliberately NOT on the site). Browser-verified: live-activity renders distinctly from placeholder; journal callout + EAL cards render; no "TBD:" left in B2–B5.
